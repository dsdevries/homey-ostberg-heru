...
export class Gen3Remote extends BaseDevice {
    ...
    setSupplyPower(power: number) {
        this.api?.writeRegister(Gen3Registers.holdingRegisters.SUPPLY_FAN_SPEED, power);
    }

    setExtractPower(power: number) {
        this.api?.writeRegister(Gen3Registers.holdingRegisters.EXHAUST_FAN_SPEED, power);
    }
